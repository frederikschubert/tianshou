...
class TimingTreeNode:
    ...
    timing: typing.OrderedDict[str, Any] = dataclasses.field(
...
class TimingContext:
    ...
    def __exit__(self, type_, value, traceback):
        time_passed = max(
            time.time() - self._time_enter, 1e-8
...
class Timing(AttrDict):
    ...
    @staticmethod
    def _time_str(value):
        return (f"{value:.4f}" if isinstance(value, float) else str(value)) + "s"
    ...
    def tree_str(self):
        lines = [f"{self._name} timing tree view:"]
