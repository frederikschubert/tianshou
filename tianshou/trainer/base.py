...
import numpy as np
from tianshou.utils.timing import Timing
import tqdm
...
class BaseTrainer(ABC):
    ...
    def __init__(
        ...
        self.iter_num = 0
        self.timing = Timing("Trainer")
        atexit.register(lambda: print(self.timing))

    def reset(self) -> None:
        """Initialize or reset the instance to yield a new iterator from zero."""
        with self.timing.time_avg("reset"):
            self.is_run = False
            self.env_step = 0
    ...
    def __next__(self) -> Union[None, Tuple[int, Dict[str, Any], Dict[str, Any]]]:
        ...
        with self.timing.time_avg("epoch") as timing_epoch:
            self.epoch += 1
            self.iter_num += 1
            ...
            with progress(
                ...
                while t.n < t.total and not self.stop_fn_flag:
                    ...
                    with self.timing.time_avg("policy_update"):
                        self.policy_update_fn(data, result)
                    
                    eta_seconds = timing_epoch.timing_tree_node.self_time.value() * (self.max_epoch - self.epoch)
                    if eta_seconds > 0:
    ...
    def test_step(self) -> Tuple[Dict[str, Any], bool]:
        ...
        with self.timing.time_avg("test_step"):
            assert self.episode_per_test is not None
    ...
    def train_step(self) -> Tuple[Dict[str, Any], Dict[str, Any], bool]:
        ...
        with self.timing.time_avg("train_step"):
            assert self.episode_per_test is not None
